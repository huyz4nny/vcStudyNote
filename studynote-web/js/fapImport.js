window.FapImportModule = {
  ...
  async doImport() {
    ...
    if (curriculumText) {
      ...
      try {
        await ApiClient.syncCurriculum(AppData.curriculum());
      } catch (err) {
        toast('❌ Lỗi đồng bộ Khung chương trình: ' + err.message, true);
      }
    }
    ...
    if (subjectsAdded > 0 && (hasPdf || transcriptText)) {
      try {
        const syncReq = { 
          subjects: AppData.subjects().map(s => ({
            ...
            passThreshold: s.passThreshold || 5,
            isCountedInGPA: s.isCountedInGPA !== false,
            gradeItems: AppData.gradeItems().filter(g => g.subjectId === s.id).map(g => ({
              category: g.category,
              name: g.name,
              weight: g.weight,
              value: g.value,
        ...
        console.log('[Import] Syncing', syncReq.subjects.length, 'subjects to API...');
        await ApiClient.syncTranscript(syncReq);
        console.log('[Import] Sync complete!');
      } catch (err) {
        console.error('[Import] Sync failed:', err);
        toast('❌ Lỗi đồng bộ Bảng điểm: ' + err.message, true);
      }
    }

    if (typeof showLoadingOverlay === 'function') showLoadingOverlay(false);
    await this.finishImport(subjectsAdded);
  },

  async finishImport(subjectsAdded) {
    if (subjectsAdded > 0) {
      // Reload fresh data from API to get correct server-assigned IDs
      try {
        console.log('[Import] Reloading subjects from API...');
        const freshSubjects = await ApiClient.getSubjects();
        console.log('[Import] Got', (freshSubjects || []).length, 'subjects from API');
        _apiStore.subjects = freshSubjects || [];
        // Re-extract grade items from fresh subjects
        let allGrades = [];
        _apiStore.subjects.forEach(s => {
          if (s.gradeItems) allGrades.push(...s.gradeItems);
        });
        _apiStore.gradeItems = allGrades;
        console.log('[Import] Total grade items:', allGrades.length);
      } catch (e) {
        console.error('[Import] Failed to reload after import:', e);
      }
